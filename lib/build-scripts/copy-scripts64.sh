...
if [ $WAX_COMPILE_SCRIPTS ]; then
  echo "note: Wax is using compiled 64 bit Lua scripts."
  ...
  lua64 "$PROJECT_DIR/wax/lib/build-scripts/luac64.lua" wax wax64.dat "$PROJECT_DIR/wax/lib/stdlib/" "$PROJECT_DIR/wax/lib/stdlib/init.lua" -L "$PROJECT_DIR/wax/lib/stdlib"/**/*.lua
  lua64 "$PROJECT_DIR/wax/lib/build-scripts/luac64.lua" "" AppDelegate.dat "$SOURCE_SCRIPTS_DIR/" "$SOURCE_SCRIPTS_DIR/AppDelegate.lua" -L "$SOURCE_SCRIPTS_DIR"/**/*.lua
...
#define WAX_STDLIB {$(hexdump -v -e '1/1 "%d,"' wax64.dat)}
EOF

cat > "$PROJECT_DIR/wax/lib/wax_stdlib64.h" <<EOF
// DO NOT MODIFY
// This is auto generated, it contains a compiled version of the wax stdlib
#define WAX_STDLIB {$(hexdump -v -e '1/1 "%d,"' wax64.dat)}
EOF

#mv wax64.dat "$DESTINATION_SCRIPTS_DIR"

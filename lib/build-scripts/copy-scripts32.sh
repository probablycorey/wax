...
if [ $WAX_COMPILE_SCRIPTS ]; then
  ...
  lua32 "$PROJECT_DIR/wax/lib/build-scripts/luac32.lua" wax wax32.dat "$PROJECT_DIR/wax/lib/stdlib/" "$PROJECT_DIR/wax/lib/stdlib/init.lua" -L "$PROJECT_DIR/wax/lib/stdlib"/**/*.lua
  lua32 "$PROJECT_DIR/wax/lib/build-scripts/luac32.lua" "" AppDelegate.dat "$SOURCE_SCRIPTS_DIR/" "$SOURCE_SCRIPTS_DIR/AppDelegate.lua" -L "$SOURCE_SCRIPTS_DIR"/**/*.lua
...
#define WAX_STDLIB {$(hexdump -v -e '1/1 "%d,"' wax32.dat)}
EOF

cat > "$PROJECT_DIR/wax/lib/wax_stdlib32.h" <<EOF
// DO NOT MODIFY
// This is auto generated, it contains a compiled version of the wax stdlib
#define WAX_STDLIB {$(hexdump -v -e '1/1 "%d,"' wax32.dat)}
EOF
